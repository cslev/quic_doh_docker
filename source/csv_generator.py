...
for f in files :
  if(re.search("^capture-[0-9]*-[0-9]*-[0-9]*",f)) is not None: #regexp for doh_docker specific capture files only
    ...
    # if(TSO_ON):
        # extra_filter=' '
    csv_command = 'tshark -r ' + file_name +' -Y "(http2)||(dns and tls)||(quic)||(dns)" -o tls.keylog_file:'+ SSLKEY +' -T fields -e frame.number -e _ws.col.Time -e ip.src -e ip.dst -e tcp.srcport -e tcp.dstport -e _ws.col.Protocol -e frame.len -e _ws.col.Info -E header=y -E separator="," -E quote=d -E occurrence=f > '+ output_file_name
